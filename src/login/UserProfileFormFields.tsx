...
export default function UserProfileFormFields(props: UserProfileFormFieldsProps<KcContext, I18n>) {
    ...
    const orderedFields = [...formFieldStates].sort((a, b) => {
        const order = ["username", "firstName", "lastName", "email"];

        const aIndex = order.indexOf(a.attribute.name);
        const bIndex = order.indexOf(b.attribute.name);
            ...
            {orderedFields.map(({ attribute, displayableErrors, valueOrValues }) => {
                return (
                    <Fragment key={attribute.name}>
                        ...
                        ${["firstName", "lastName", "password", "password-confirm"].includes(attribute.name) ? "col-span-1" : "col-span-2"}`}
                            style={{
                                display:
